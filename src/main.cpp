...
int main()
{
  ...
  PID pid;
  // pid.Init(0.16, 1E-7, 0.8); //inital values
  pid.Init(0.1, 0.002, 7.5);

  PID throttle;
  throttle.Init(0.2, 0.0, 0.13);
      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          */
          ...
          throttle.UpdateError(fabs(cte));
          double throttle_value = 2.5 * ((25 - fabs(angle)) / 100);
          cout << throttle_value << endl;
